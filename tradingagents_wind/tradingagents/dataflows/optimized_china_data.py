...
class OptimizedChinaDataProvider:
    ...
    def get_stock_data(self, symbol: str, start_date: str, end_date: str, 
        ...
        if not force_refresh:
            # 按优先级顺序查找缓存：Wind > AKShare > Tushare
            cache_sources = ["wind", "akshare", "tushare", "unified"]
            for source in cache_sources:
                ...
                if cache_key:
                    ...
                    if cached_data:
                        logger.info(f"⚡ 从{source}缓存加载A股数据: {symbol}")
                        return cached_data
        
        # 缓存未命中，从数据源获取
        logger.info(f"🌐 从数据源获取数据: {symbol}")
        
        try:
            # API限制处理
            self._wait_for_rate_limit()
            
            # 调用统一数据源接口（优先Wind，支持备用数据源）
            ...
            # 确定实际使用的数据源并保存到缓存
            actual_source = self._determine_actual_data_source(formatted_data)
            ...
            logger.info(f"✅ A股数据获取成功: {symbol} (数据源: {actual_source})")
            return formatted_data
            
        except Exception as e:
            error_msg = f"数据接口调用异常: {str(e)}"
    ...
    def get_fundamentals_data(self, symbol: str, force_refresh: bool = False) -> str:
        ...
        # 检查缓存（除非强制刷新）
        if not force_refresh:
            # 按优先级顺序查找基本面数据缓存：Wind > AKShare > Tushare
            cache_sources = ["wind", "akshare", "tushare", "wind_analysis"]
        ...
        try:
            ...
            # 确定数据源并保存到缓存
            actual_source = self._determine_fundamentals_data_source(fundamentals_data)
            ...
            logger.info(f"✅ A股基本面数据生成成功: {symbol} (数据源: {actual_source})")
...
"""
    ...
    def _get_real_financial_metrics(self, symbol: str, price_value: float) -> dict:
        """获取真实财务指标 - 优先使用Wind数据源（Wind > AKShare > Tushare）"""
        try:
            # 🔄 优先尝试Wind数据源
            logger.info(f"🔄 优先尝试Wind获取{symbol}财务数据")
            try:
                from .wind_adapter import get_wind_adapter
                wind_adapter = get_wind_adapter()
                
                if wind_adapter and wind_adapter.provider and wind_adapter.provider.connected:
                    # 尝试通过Wind provider获取财务数据
                    financial_data = wind_adapter.provider.get_financial_data(symbol)
                    if financial_data:
                        logger.info(f"✅ Wind财务数据获取成功: {symbol}")
                        # 解析Wind财务数据
                        metrics = self._parse_wind_financial_data(financial_data, price_value)
                        if metrics:
                            logger.info(f"✅ Wind财务数据解析成功: {symbol}")
                            return metrics
                        else:
                            logger.warning(f"⚠️ Wind财务数据解析失败，尝试AKShare: {symbol}")
                    else:
                        logger.warning(f"⚠️ Wind未获取到{symbol}财务数据，尝试AKShare")
                else:
                    logger.warning(f"⚠️ Wind适配器不可用或未连接，尝试AKShare")
            except Exception as e:
                logger.warning(f"⚠️ Wind财务数据获取失败，尝试AKShare: {e}")
            ...
            if akshare_provider.connected:
                ...
                if financial_data and any(not v.empty if hasattr(v, 'empty') else bool(v) for v in financial_data.values()):
                    ...
                    if metrics:
                        ...
                        logger.warning(f"⚠️ AKShare解析失败，尝试Tushare")
                else:
    ...
    def _parse_wind_financial_data(self, financial_data: dict, price_value: float) -> dict:
        """解析Wind财务数据为指标"""
        try:
            logger.info(f"🔧 开始解析Wind财务数据...")
            
            # Wind财务数据结构: {'balance_sheet': [], 'income_statement': [], 'cash_flow': []}
            metrics = {}
            ...
            # 基本估值指标
            # 计算PE比率（基于净利润）
            net_profit = latest_income.get('net_profit_is', 0) if latest_income else 0
            if net_profit and net_profit > 0:
                # 估算市值（简化计算）
                market_cap = price_value * 1000000000  # 假设10亿股本
                pe_val = market_cap / (net_profit * 10000)  # 转换单位
                metrics["pe"] = f"{pe_val:.1f}倍"
            else:
                metrics["pe"] = "N/A"
                
            # 计算PB比率（基于股东权益）
            shareholders_equity = latest_balance.get('wgsd_stkhldrs_eq', 0) if latest_balance else 0
            if shareholders_equity and shareholders_equity > 0:
                market_cap = price_value * 1000000000
                pb_val = market_cap / (shareholders_equity * 10000)
                metrics["pb"] = f"{pb_val:.2f}倍"
            else:
                metrics["pb"] = "N/A"
                
            # 计算PS比率（基于营业收入）
            total_revenue = latest_income.get('tot_oper_rev', 0) if latest_income else 0
            if total_revenue and total_revenue > 0:
                market_cap = price_value * 1000000000
                ps_val = market_cap / (total_revenue * 10000)
                metrics["ps"] = f"{ps_val:.1f}倍"
            else:
                metrics["ps"] = "N/A"
            ...
            if net_profit and shareholders_equity and shareholders_equity > 0:
                roe_val = (net_profit / shareholders_equity) * 100
                metrics["roe"] = f"{roe_val:.1f}%"
            else:
                metrics["roe"] = "N/A"
                
            # ROA计算（净利润/总资产）
            total_assets = latest_balance.get('tot_assets', 0) if latest_balance else 0
            if net_profit and total_assets and total_assets > 0:
                roa_val = (net_profit / total_assets) * 100
                metrics["roa"] = f"{roa_val:.1f}%"
            else:
                metrics["roa"] = "N/A"
            
            # 利润率指标
            # 毛利率计算（（营业收入-营业成本）/营业收入）
            total_cost = latest_income.get('tot_oper_cost', 0) if latest_income else 0
            if total_revenue and total_revenue > 0:
                gross_profit = total_revenue - total_cost if total_cost else total_revenue
                gross_margin_val = (gross_profit / total_revenue) * 100
            ...
            if net_profit and total_revenue and total_revenue > 0:
                net_margin_val = (net_profit / total_revenue) * 100
            ...
            # 财务健康度指标
            # 资产负债率计算（总负债/总资产）
            total_liab = latest_balance.get('tot_liab', 0) if latest_balance else 0
            if total_assets and total_assets > 0:
                debt_ratio_val = (total_liab / total_assets) * 100
                metrics["debt_ratio"] = f"{debt_ratio_val:.1f}%"
            else:
                metrics["debt_ratio"] = "N/A"
            
            # 其他指标设为默认值（Wind数据可能不包含这些字段）
            metrics.update({
                "current_ratio": "N/A",
                "quick_ratio": "N/A",
                "dividend_yield": "N/A",
            ...
            logger.info(f"✅ Wind财务数据解析成功: PE={metrics['pe']}, PB={metrics['pb']}, ROE={metrics['roe']}")
            return metrics
            
        except Exception as e:
            logger.error(f"❌ Wind财务数据解析失败: {e}")
            return None
...
- 风险承受能力较低的投资者应避免"""
    ...
    def _try_get_old_cache(self, symbol: str, start_date: str, end_date: str) -> Optional[str]:
        """尝试获取过期的缓存数据作为备用，按优先级查找（Wind > AKShare > Tushare）"""
        try:
            # 按优先级顺序查找过期缓存：Wind > AKShare > Tushare > unified
            cache_sources = ["wind", "akshare", "tushare", "unified"]
            
            for source in cache_sources:
                logger.debug(f"🔍 尝试查找{source}过期缓存...")
                ...
                for metadata_file in self.cache.metadata_dir.glob(f"*_meta.json"):
                    try:
                        ...
                        if (metadata.get('symbol') == symbol and 
                            ...
                            cached_data = self.cache.load_stock_data(cache_key)
                            if cached_data:
                                logger.info(f"📁 找到{source}过期缓存数据: {symbol}")
                                return cached_data + f"\n\n⚠️ 注意: 使用的是{source}过期缓存数据"
                    except Exception:
                        continue
                        
            logger.debug(f"🔍 未找到任何可用的过期缓存数据: {symbol}")
        except Exception as e:
            logger.debug(f"⚠️ 查找过期缓存失败: {e}")
...
"""
    ...
    def _determine_actual_data_source(self, data: str) -> str:
        """根据数据内容确定实际使用的数据源"""
        if "Wind数据分析报告" in data or "数据来源: Wind" in data:
            return "wind"
        elif "Tushare数据" in data or "数据来源: Tushare" in data:
            return "tushare"
    ...
    def _determine_fundamentals_data_source(self, fundamentals_data: str) -> str:
        """根据基本面数据内容确定实际使用的数据源"""
        if "Wind" in fundamentals_data and "data_source" in fundamentals_data:
            return "wind"
        elif "AKShare" in fundamentals_data:
            return "akshare"
        elif "Tushare" in fundamentals_data:
            return "tushare"
        else:
            return "wind_analysis"  # 默认分析数据源
