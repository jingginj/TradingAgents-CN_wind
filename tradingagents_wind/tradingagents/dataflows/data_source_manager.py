...
class DataSourceManager:
    ...
    def _get_default_source(self) -> ChinaDataSource:
        ...
        env_source = os.getenv('DEFAULT_CHINA_DATA_SOURCE', 'wind').lower()

        # 映射到枚举
        source_mapping = {
            'wind': ChinaDataSource.WIND,
        ...
        return source_mapping.get(env_source, ChinaDataSource.WIND)
    ...
    def _check_available_sources(self) -> List[ChinaDataSource]:
        ...
        try:
            ...
            if start_result.ErrorCode == 0:
                available.append(ChinaDataSource.WIND)
                logger.info("✅ Wind数据源可用")
                # 立即关闭测试连接，避免与后续实际使用时的连接冲突
                try:
                    w.stop()
                    logger.debug("🔄 Wind测试连接已关闭")
                except Exception as e:
                    logger.warning(f"⚠️ Wind测试连接关闭失败: {e}")
            else:
                logger.warning(f"⚠️ Wind数据源不可用: 连接失败，错误代码 {start_result.ErrorCode}")
        except ImportError:
            logger.warning("⚠️ Wind数据源不可用: WindPy库未安装")
        except Exception as e:
            logger.warning(f"⚠️ Wind数据源不可用: 连接异常 {e}")
    ...
    def get_data_adapter(self):
        """获取当前数据源的适配器"""
        if self.current_source == ChinaDataSource.WIND:
            return self._get_wind_adapter()
    ...
    def _get_wind_adapter(self):
        """获取Wind适配器"""
        try:
            from .wind_adapter import get_wind_adapter
            return get_wind_adapter()
        except ImportError as e:
            logger.error(f"❌ Wind适配器导入失败: {e}")
            return None
    ...
    def get_stock_data(self, symbol: str, start_date: str = None, end_date: str = None) -> str:
        ...
        try:
            # 根据数据源调用相应的获取方法
            if self.current_source == ChinaDataSource.WIND:
                logger.info(f"🔍 [股票代码追踪] 调用 Wind 数据源，传入参数: symbol='{symbol}'")
                result = self._get_wind_data(symbol, start_date, end_date)
            elif self.current_source == ChinaDataSource.TUSHARE:
                logger.info(f"🔍 [股票代码追踪] 调用 Tushare 数据源，传入参数: symbol='{symbol}'")
                result = self._get_tushare_data(symbol, start_date, end_date)
            elif self.current_source == ChinaDataSource.AKSHARE:
                logger.info(f"🔍 [股票代码追踪] 调用 akshare 数据源，传入参数: symbol='{symbol}'")
    ...
    def _get_wind_data(self, symbol: str, start_date: str, end_date: str) -> str:
        """使用Wind获取数据"""
        logger.debug(f"📊 [Wind] 调用参数: symbol={symbol}, start_date={start_date}, end_date={end_date}")

        # 添加详细的股票代码追踪日志
        logger.info(f"🔍 [股票代码追踪] _get_wind_data 接收到的股票代码: '{symbol}' (类型: {type(symbol)})")
        logger.info(f"🔍 [股票代码追踪] 股票代码长度: {len(str(symbol))}")
        logger.info(f"🔍 [股票代码追踪] 股票代码字符: {list(str(symbol))}")
        logger.info(f"🔍 [DataSourceManager详细日志] _get_wind_data 开始执行")
        logger.info(f"🔍 [DataSourceManager详细日志] 当前数据源: {self.current_source.value}")

        start_time = time.time()
        try:
            # 调用Wind数据接口函数
            result = get_china_stock_data_wind_impl(symbol, start_date, end_date)

            duration = time.time() - start_time
            logger.info(f"🔍 [DataSourceManager详细日志] wind接口调用完成，耗时: {duration:.3f}秒")
            logger.info(f"🔍 [股票代码追踪] get_china_stock_data_wind_impl 返回结果前200字符: {result[:200] if result else 'None'}")
            ...
            logger.debug(f"📊 [Wind] 调用完成: 耗时={duration:.2f}s, 结果长度={len(result) if result else 0}")

            return result
        except Exception as e:
            duration = time.time() - start_time
            logger.error(f"❌ [Wind] 调用失败: {e}, 耗时={duration:.2f}s", exc_info=True)
    ...
    def _try_fallback_sources(self, symbol: str, start_date: str, end_date: str) -> str:
        ...
        # 备用数据源优先级: Wind > AKShare > Tushare > BaoStock > TDX
        fallback_order = [
            ChinaDataSource.WIND,
        ...
        for source in fallback_order:
            if source != self.current_source and source in self.available_sources:
                try:
                    ...
                    # 直接调用具体的数据源方法，避免递归
                    if source == ChinaDataSource.WIND:
                        result = self._get_wind_data(symbol, start_date, end_date)
    ...
    def get_stock_info(self, symbol: str) -> Dict:
        ...
        try:
            if self.current_source == ChinaDataSource.WIND:
                # 调用Wind股票信息接口函数
                info_str = get_china_stock_info_wind_impl(symbol)
                result = self._parse_stock_info_string(info_str, symbol)

                # 检查是否获取到有效信息
                if result.get('name') and result['name'] != f'股票{symbol}':
                    logger.info(f"✅ [股票信息] Wind成功获取{symbol}信息")
                    return result
                else:
                    logger.warning(f"⚠️ [股票信息] Wind返回无效信息，尝试降级...")
...
def get_china_stock_data_wind_impl(
    ticker: str,
    start_date: str,
    end_date: str
) -> str:
    """
    使用Wind获取中国A股历史数据的具体实现
    ...
    try:
        ...
        logger.debug(f"📊 [Wind] 获取{ticker}股票数据...")

        # 添加详细的股票代码追踪日志
        logger.info(f"🔍 [股票代码追踪] get_china_stock_data_wind_impl 接收到的股票代码: '{ticker}' (类型: {type(ticker)})")
        logger.info(f"🔍 [股票代码追踪] 股票代码长度: {len(str(ticker))}")
        logger.info(f"🔍 [股票代码追踪] 股票代码字符: {list(str(ticker))}")

        adapter = get_wind_adapter()
        ...
        if data is not None and not data.empty:
            ...
            price_change = latest_price - data.iloc[-2]['close'] if len(data) > 1 else 0
            price_change_pct = (price_change / data.iloc[-2]['close'] * 100) if len(data) > 1 and data.iloc[-2]['close'] != 0 else 0
            ...
            try:
                if hasattr(data.index[-1], 'strftime'):
                    ...
                    latest_date = str(data.iloc[-1]['date'])[:10]  # 取前10个字符 YYYY-MM-DD
                elif 'trade_date' in data.columns:
                    latest_date = str(data.iloc[-1]['trade_date'])[:10]
                else:
                    latest_date = end_date  # 使用结束日期作为备选
            except Exception:
...
• 价格波动率: {data['close'].std():.2f}

数据来源: Wind
数据获取时间: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}
"""
            logger.info(f"✅ [Wind] 成功获取{ticker}数据，数据点数: {len(data)}")
            return report
        else:
            logger.warning(f"⚠️ [Wind] 未获取到{ticker}的有效数据")
            return f"未获取到股票 {ticker} 的数据，请检查股票代码或网络连接。"

    except Exception as e:
        logger.error(f"❌ [Wind] 获取{ticker}数据失败: {e}")
        return f"获取股票 {ticker} 数据时发生错误: {str(e)}"


def search_china_stocks_wind_impl(
    keyword: str
) -> str:
    """
    使用Wind搜索中国A股股票的具体实现
    ...
    try:
        ...
        logger.debug(f"🔍 [Wind] 搜索股票: {keyword}")
        ...
        logger.error(f"❌ [Wind] 搜索股票失败: {e}")
        return f"❌ 搜索股票失败: {e}"


def get_china_stock_fundamentals_wind_impl(
    ticker: str
) -> str:
    """
    使用Wind获取中国A股基本面数据的具体实现
    ...
    try:
        ...
        logger.debug(f"📊 [Wind] 获取{ticker}基本面数据...")
        ...
        logger.error(f"❌ [Wind] 获取基本面数据失败: {e}")
        return f"❌ 获取{ticker}基本面数据失败: {e}"


def get_china_stock_info_wind_impl(
    ticker: str
) -> str:
    """
    使用Wind获取中国A股基本信息的具体实现
    ...
    try:
        ...
        logger.debug(f"📊 [Wind] 获取{ticker}基本信息...")
        ...
        if info and info.get('name'):
            ...
            result += f"数据来源: {info.get('source', 'wind')}\n"
        ...
        logger.error(f"❌ [Wind] 获取基本信息失败: {e}")
        return f"❌ 获取{ticker}基本信息失败: {e}"
