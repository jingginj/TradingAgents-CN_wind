...
def run_stock_analysis(stock_symbol, analysis_date, analysts, research_depth, llm_provider, llm_model, market_type="美股", progress_callback=None):
    ...
    try:
        ...
        # 🧹 重要：分析完成后清理Wind资源，避免与Streamlit冲突
        try:
            from tradingagents.dataflows.interface import cleanup_all_resources
            cleanup_result = cleanup_all_resources()
            logger.info(f"🧹 分析完成，资源清理结果: {cleanup_result['success']}/{cleanup_result['total']} 项成功")
        except Exception as cleanup_error:
            logger.warning(f"⚠️ 分析完成后资源清理失败: {cleanup_error}")
        ...
        # 🧹 即使分析失败也要清理Wind资源，避免资源泄漏
        try:
            from tradingagents.dataflows.interface import cleanup_all_resources
            cleanup_result = cleanup_all_resources()
            logger.info(f"🧹 分析异常，资源清理结果: {cleanup_result['success']}/{cleanup_result['total']} 项成功")
        except Exception as cleanup_error:
            logger.warning(f"⚠️ 分析异常后资源清理失败: {cleanup_error}")
